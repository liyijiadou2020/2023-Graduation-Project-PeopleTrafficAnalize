...
if __name__ == '__main__':
    ...
    # todo: 首先要删除已存在的query_features.npy和names.npy

    for image_name in os.listdir('../query'):
        img = cv2.imread(os.path.join('../query', image_name))

        t1 = time.time()
        feat = Reid_feature(img)   # normlized feat，调用了__call__(list_img)
        print('spend pytorch time: ', time.time() - t1)
        pytorch_output = feat.numpy()
        embs = np.concatenate((pytorch_output, embs), axis=0)
        # embs.append(pytorch_output)
        names.append(image_name[0:-4]) #不要.jpg
