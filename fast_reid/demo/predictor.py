...
class FeatureExtractionDemo(object):
    ...
    def run_on_batch(self, img_list):
        t1 = time.time()
        # original_image = [original_image[:, :, ::-1] for original_image in img_list]  # RGB
        # image = [cv2.resize(i, tuple(self.cfg.INPUT.SIZE_TEST[::-1]), interpolation=cv2.INTER_CUBIC) for i in original_image]
        # batch_image = [torch.as_tensor(img.astype("float32").transpose(2, 0, 1))[None] for img in image]
        # # print(len(batch_image), batch_image[0].shape)
